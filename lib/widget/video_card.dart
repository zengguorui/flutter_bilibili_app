...
class VideoCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return InkWell(
      onTap: (){
        print(videoMo.url);
        HiNavigator.getInstance().onJumpTo(RouteStatus.detail, args: {"videoModel":videoMo});
      },
      child: SizedBox(
        height: 200,
        child: Card(
  ...
  _infoText(Color textColor) {
    return Expanded(child: Container(
      ...
      child: Column(
        ...
        mainAxisAlignment: MainAxisAlignment.spaceBetween,
        children: [
          Text(videoMo.title, style: TextStyle(fontSize: 12, color: textColor), maxLines: 2, overflow: TextOverflow.ellipsis,),
          _onwer(textColor),
        ],
      ),
    ));
        ...
        Row(
          children: [
            ...
            Padding(padding: EdgeInsets.only(left: 8),
              child: Text(owner.name, style: TextStyle(fontSize: 11, color: textColor),),
            )
          ],
        ),
